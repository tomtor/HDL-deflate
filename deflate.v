...
reg [18:0] leaves [0:2048-1];
reg [7:0] iram [0:16384-1];
reg [3:0] distanceLength [0:32-1];
reg [18:0] d_leaves [0:512-1];
reg [10:0] code_bits [0:288-1];
...
begin: MYHDL149_RETURN
    MYHDL58_get_bits = ($signed({1'b0, aleaf}) & ((1 << 4) - 1));
...
begin: MYHDL151_RETURN
    MYHDL60_get_code = (aleaf >>> 4);
    disable MYHDL151_RETURN;
end
endfunction

function integer MYHDL61_get4;
    input boffset;
    integer boffset;
    input [5-1:0] width;
begin: MYHDL152_RETURN
    ...
    MYHDL61_get4 = ($signed($signed({1'b0, b41}) >>> ($signed({1'b0, dio}) + boffset)) & ((1 << width) - 1));
    disable MYHDL152_RETURN;
end
endfunction

function integer MYHDL62_get_bits;
    input [19-1:0] aleaf;
begin: MYHDL153_RETURN
    MYHDL62_get_bits = ($signed({1'b0, aleaf}) & ((1 << 4) - 1));
...
begin: MYHDL154_RETURN
    ...
    MYHDL63_get4 = ((b41 >>> (dio + boffset)) & ((1 << width) - 1));
    disable MYHDL154_RETURN;
end
endfunction

function integer MYHDL64_get_code;
    input [19-1:0] aleaf;
begin: MYHDL155_RETURN
    MYHDL64_get_code = (aleaf >>> 4);
...
begin: MYHDL156_RETURN
    MYHDL65_get_bits = ($signed({1'b0, aleaf}) & ((1 << 4) - 1));
...
function integer MYHDL71_rev_bits;
    input b;
    integer b;
    input nb;
    integer nb;
        ...
        case (state)
            ...
            5'b11000: begin
                if ((1'b1 && 1'b1)) begin
                    ...
                    lencode = 257;
                    match = 3;
                    if ((($signed({1'b0, di}) < ($signed({1'b0, isize}) - 4)) && (iram[(fmatch2 & 16383)] == b4))) begin
                        lencode = 258;
                        match = 4;
                        if ((($signed({1'b0, di}) < ($signed({1'b0, isize}) - 5)) && (iram[((fmatch2 + 1) & 16383)] == b5))) begin
                            lencode = 259;
                            match = 5;
                            if ((1'b1 && ($signed({1'b0, di}) < ($signed({1'b0, isize}) - 6)) && (iram[((fmatch2 + 2) & 16383)] == b6))) begin
                                lencode = 260;
                                match = 6;
                                if ((($signed({1'b0, di}) < ($signed({1'b0, isize}) - 7)) && (iram[((fmatch2 + 3) & 16383)] == b7))) begin
                                    lencode = 261;
                                    match = 7;
                                    if ((($signed({1'b0, di}) < ($signed({1'b0, isize}) - 8)) && (iram[((fmatch2 + 4) & 16383)] == b8))) begin
                                        lencode = 262;
                                        match = 8;
                                        if ((($signed({1'b0, di}) < ($signed({1'b0, isize}) - 9)) && (iram[((fmatch2 + 5) & 16383)] == b9))) begin
                                            lencode = 263;
                                            match = 9;
                                            if ((($signed({1'b0, di}) < ($signed({1'b0, isize}) - 10)) && (iram[((fmatch2 + 6) & 16383)] == b10))) begin
                                                lencode = 264;
                                                match = 10;
                                            end
                                        end
            ...
            5'b10111: begin
                if ((!1'b1)) begin
                    ...
                    if (((cur_search >= 0) && (cur_search >= ($signed({1'b0, di}) - 32)) && ($signed({1'b0, di}) < ($signed({1'b0, isize}) - 3)))) begin
                        ...
                        else if (((!1'b1) && (iram[(cur_search & 16383)] == b1) && (iram[((cur_search + 1) & 16383)] == b2) && (iram[((cur_search + 2) & 16383)] == b3))) begin
                            lencode = 257;
                            match = 3;
                            if ((($signed({1'b0, di}) < ($signed({1'b0, isize}) - 4)) && (iram[((cur_search + 3) & 16383)] == b4))) begin
                                lencode = 258;
                                match = 4;
                                if ((($signed({1'b0, di}) < ($signed({1'b0, isize}) - 5)) && (iram[((cur_search + 4) & 16383)] == b5))) begin
                                    lencode = 259;
                                    match = 5;
                                    if ((1'b1 && ($signed({1'b0, di}) < ($signed({1'b0, isize}) - 6)) && (iram[((cur_search + 5) & 16383)] == b6))) begin
                                        lencode = 260;
                                        match = 6;
                                        if ((($signed({1'b0, di}) < ($signed({1'b0, isize}) - 7)) && (iram[((cur_search + 6) & 16383)] == b7))) begin
                                            lencode = 261;
                                            match = 7;
                                            if ((($signed({1'b0, di}) < ($signed({1'b0, isize}) - 8)) && (iram[((cur_search + 7) & 16383)] == b8))) begin
                                                lencode = 262;
                                                match = 8;
                                                if ((($signed({1'b0, di}) < ($signed({1'b0, isize}) - 9)) && (iram[((cur_search + 8) & 16383)] == b9))) begin
                                                    lencode = 263;
                                                    match = 9;
                                                    if ((($signed({1'b0, di}) < ($signed({1'b0, isize}) - 10)) && (iram[((cur_search + 9) & 16383)] == b10))) begin
                                                        lencode = 264;
                                                        match = 10;
                                                    end
                                                end
            ...
            5'b00111: begin
                ...
                if ((1'b1 && (cur_HF1 < 512))) begin
                    d_leaves[cur_HF1] <= 0;
                end
                if (((method != 4) && (cur_HF1 < 2048))) begin
                    leaves[cur_HF1] <= 0;
                end
                limit = 2048;
                if ((method == 4)) begin
            ...
            5'b10010: begin
                if (((method == 4) && 1'b1)) begin
                    ...
                    leaves[spread] <= MYHDL55_makeLeaf(spread_i, codeLength[spread_i]);
                end
                aim = instantMask;
                if ((method == 4)) begin
                    aim = d_instantMask;
            ...
            5'b01111: begin
                if ((!1'b1)) begin
                    ...
                    endcase
                    cto = MYHDL61_get4(extraLength, d_maxBits);
            ...
            5'b10000: begin
                if (1'b1) begin
                    if ((MYHDL62_get_bits(leaf) == 0)) begin
                        $finish;
                    end
                    token = (code - 257);
                    case (token)
                    ...
                    case (token)
                        ...
                        default: extraLength = 0;
                    endcase
                    tlength = tlength + MYHDL63_get4(0, extraLength);
                    distanceCode = MYHDL64_get_code(leaf);
                    case (distanceCode)
                        0: distance = 1;
                        1: distance = 2;
                        2: distance = 3;
                    ...
                    case ($signed(distanceCode >>> 1))
                        ...
                        default: moreBits = 13;
                    endcase
                    mored = MYHDL66_get4((extraLength + MYHDL65_get_bits(leaf)), moreBits);
                    distance = distance + mored;
                    if ((distance > $signed({1'b0, do}))) begin
                        $finish;
                    end
                    MYHDL68_adv(((moreBits + extraLength) + MYHDL67_get_bits(leaf)));
                    offset <= (($signed({1'b0, do}) - distance) & 32767);
                    length <= tlength;
                    cur_i <= 0;
                    oraddr <= (($signed({1'b0, do}) - distance) & 32767);
                    state <= 5'b10101;
                end
            end
            5'b10100: begin
                    ...
                    if ((code < 256)) begin
                        ...
                        if (static) begin
                            ...
                            case (token)
                                ...
                                default: extraLength = 0;
                            endcase
                            tlength = tlength + MYHDL69_get4(0, extraLength);
                            t = MYHDL70_get4(extraLength, 5);
                            distanceCode = MYHDL71_rev_bits(t, 5);
                            case (distanceCode)
                                0: distance = 1;
                                1: distance = 2;
                                2: distance = 3;
                            ...
                            case ($signed(distanceCode >>> 1))
                                ...
                                default: moreBits = 13;
                            endcase
                            distance = distance + MYHDL72_get4((extraLength + 5), moreBits);
                            MYHDL73_adv(((extraLength + 5) + moreBits));
            ...
            5'b10101: begin
                if ((!1'b1)) begin
                    ...
                    if ((cur_i > 1)) begin
                        ...
                        else if (((cur_i == 3) || ((offset + cur_i) != do))) begin
                            obyte <= orbyte;
                        end
