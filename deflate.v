                    ...
                    if (((cur_search >= 0) && (cur_search >= ($signed({1'b0, di}) - 32)) && ($signed({1'b0, di}) < ($signed({1'b0, isize}) - 3)))) begin
                        ...
                        if (1'b1) begin
                            ...
                            if (((!found) || ((($signed({1'b0, di}) - fmatch) - 1) < 0))) begin
                                ...
                                lencode = 257;
                                match = 3;
                                if ((1'b0 && ($signed({1'b0, di}) < ($signed({1'b0, isize}) - 4)) && (iram[(fmatch & 127)] == b4))) begin
                                    lencode = 258;
                                    match = 4;
                                    if ((1'b0 && ($signed({1'b0, di}) < ($signed({1'b0, isize}) - 5)) && (fmatch > 1) && (($signed($signed({1'b0, cwindow}) >>> (8 * (fmatch - 2))) & 255) == iram[((di + 4) & 127)]))) begin
                                        lencode = 259;
                                        match = 5;
